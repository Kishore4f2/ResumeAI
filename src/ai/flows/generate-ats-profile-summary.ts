...
const GenerateAtsProfileSummaryOutputSchema = z.object({
  profileSummary: z
    .string()
    .describe(
      `A 4-6 line profile summary tailored for fresher resumes, optimized for ATS.`
    ),
...
const profileSummaryPrompt = ai.definePrompt({
  ...
  prompt: `You are an AI resume expert. Generate a concise (4-6 line) profile summary tailored for fresher resumes based on the following job description. Optimize the summary for Applicant Tracking Systems (ATS) with a score of 70% or higher. Use the estimateAtsScore tool to validate the ATS score and improve the summary if needed.

Job Description: {{{jobDescription}}}

Profile Summary:`, // Removed unnecessary prompt
});
